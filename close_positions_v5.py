#!/usr/bin/env python3
"""
Закрытие позиций L1 Bot через правильный v5 API
Использует корректные параметры для Unified Account
"""
...
class BybitClient:
    ...
    def close_position_v5(self, symbol: str, side: str, size: str) -> Dict:
        """Закрыть позицию через v5 API"""
        endpoint = "/v5/order/create"
        
        # Инвертируем сторону для закрытия
        close_side = "Buy" if side == "Sell" else "Sell"
        
        # Правильные параметры для v5 API
        params = {
            "category": "linear",
            "symbol": symbol,
            "side": close_side,
...
# ========== ОСНОВНАЯ ЛОГИКА ==========
def main():
    print("🔓 Закрытие позиций L1 Bot через v5 API")
    print("=" * 60)
    ...
    for pos in active_positions:
        ...
        result = client.close_position_v5(symbol, side, size)
        ...
        if result.get("retCode") == 0:
            ...
            print(f"      ❌ Ошибка закрытия: {result.get('retMsg', 'Unknown')}")
